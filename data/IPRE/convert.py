...
def convert_data(dataset_name, sentence_filepath, sent_relation_filepath):
    sents = load_csv(
        sentence_filepath, title_row=False, title_keys=["id", "head", "tail", "text"]
    )
    id2sent = {}
    for ins in sents:
        id2sent[ins["id"]] = ins
    sent_labels = load_csv(
        sent_relation_filepath, title_row=False, title_keys=["id", "relations"]
    )
    final_data = []
...
if __name__ == "__main__":
    ...
    rel2ids = load_csv(
        "raw/IPRE/data/relation2id.txt", title_row=False, title_keys=["relation", "id"]
    )
    rel2id = {}
    for ins in rel2ids:
    ...
    for dn in ["train", "dev", "test"]:
        convert_data(
            dn,
            f"raw/IPRE/data/{dn}/sent_{dn}.txt",
