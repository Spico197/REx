...
def measure_triple(preds, golds, eps=1e-12):
    ...
    for pred, gold in zip(preds, golds):
        ...
        pred_relations = set(x[1] for x in pred)
        gold_relations = set(x[1] for x in gold)
        ...
        pred_subjs = set(x[0] for x in pred)
        gold_subjs = set(x[0] for x in gold)
        ...
        pred_objs = set(x[2] for x in pred)
        gold_objs = set(x[2] for x in gold)
